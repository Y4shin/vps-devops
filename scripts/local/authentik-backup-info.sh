...
borg_repo="ssh://$(
  ...
  SOPS_AGE_KEY_FILE="$age_key_file" \
    sops -d --extract '["sops_authentik_secrets"]["borg_path"]' "${repo_root}/ansible/inventories/prod/group_vars/all.sops.yaml"
)"
borg_passphrase_value="$(
  SOPS_AGE_KEY_FILE="$age_key_file" \
    sops -d --extract '["sops_authentik_secrets"]["borg_passphrase"]' "${repo_root}/ansible/inventories/prod/group_vars/all.sops.yaml"
)"
